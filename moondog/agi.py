...
class AGImessageHangup(AGIHangup):
    pass
...
class AGI(object):
    '''
        This class handles communication with Asterisk.
    '''
    ...
    def get_message(self, stdin=sys.stdin):
        '''Read the message of a command from Asterisk'''
        code = 0
        message = {'message':('','')}
        line = stdin.readline().strip()
        sys.stderr.write('    message_LINE: %s\n' % line)
        ...
        if code == 200:
            for key,value,data in re_kv.findall(response):
                ...
                if data == 'hangup':
                    raise AGImessageHangup("User hungup during execution")

                if key == 'message' and value == '-1':
                    raise AGIAppError("Error executing application, or hangup")

            sys.stderr.write('    message_DICT: %s\n' % pprint.pformat(message))
            return message
        elif code == 510:
    ...
    def wait_for_digit(self, timeout=DEFAULT_TIMEOUT):
        ...
        '''
        res = self.execute('WAIT FOR DIGIT', timeout)['message'][0]
        if res == '0':
            return ''
        else:
            try:
                ...
                raise AGIError('Unable to convert message to digit: %s' % res)
    ...
    def send_text(self, text=''):
        ...
        '''
        self.execute('SEND TEXT', self._quote(text))['message'][0]
    ...
    def receive_char(self, timeout=DEFAULT_TIMEOUT):
        ...
        '''
        res = self.execute('RECEIVE CHAR', timeout)['message'][0]
        ...
        if res == '0':
            ...
            try:
                ...
                raise AGIError('Unable to convert message to char: %s' % res)
    ...
    def tdd_mode(self, mode='off'):
        ...
        '''
        res = self.execute('TDD MODE', mode)['message'][0]
        if res == '0':
    ...
    def send_image(self, filename):
        ...
        '''
        res = self.execute('SEND IMAGE', filename)['message'][0]
        if res != '0':
    ...
    def say_digits(self, digits, escape_digits=''):
        ...
        escape_digits = self._process_digit_list(escape_digits)
        res = self.execute('SAY DIGITS', digits, escape_digits)['message'][0]
        if res == '0':
            return ''
        else:
            try:
    ...
    def say_number(self, number, escape_digits=''):
        ...
        escape_digits = self._process_digit_list(escape_digits)
        res = self.execute('SAY NUMBER', number, escape_digits)['message'][0]
        if res == '0':
            return ''
        else:
            try:
    ...
    def say_alpha(self, characters, escape_digits=''):
        ...
        escape_digits = self._process_digit_list(escape_digits)
        res = self.execute('SAY ALPHA', characters, escape_digits)['message'][0]
        if res == '0':
            return ''
        else:
            try:
    ...
    def say_phonetic(self, characters, escape_digits=''):
        ...
        characters = self._process_digit_list(characters)
        escape_digits = self._process_digit_list(escape_digits)
        res = self.execute('SAY PHONETIC', characters, escape_digits)['message'][0]
        if res == '0':
            return ''
        else:
            try:
    ...
    def say_date(self, seconds, escape_digits=''):
        ...
        '''
        escape_digits = self._process_digit_list(escape_digits)
        res = self.execute('SAY DATE', seconds, escape_digits)['message'][0]
        if res == '0':
            return ''
        else:
            try:
    ...
    def say_time(self, seconds, escape_digits=''):
        ...
        '''
        escape_digits = self._process_digit_list(escape_digits)
        res = self.execute('SAY TIME', seconds, escape_digits)['message'][0]
        if res == '0':
            return ''
        else:
            try:
    ...
    def say_datetime(self, seconds, escape_digits='', format='', zone=''):
        ...
        res = self.execute('SAY DATETIME', seconds, escape_digits, format, zone)['message'][0]
        if res == '0':
            return ''
        else:
            try:
    ...
    def get_data(self, filename, timeout=DEFAULT_TIMEOUT, max_digits=255):
        ...
        '''
        message = self.execute('GET DATA', filename, timeout, max_digits)
    ...
    def record_file(self, filename, format='gsm', escape_digits='#', timeout=DEFAULT_RECORD, offset=0, beep='beep'):
        ...
        '''
        escape_digits = self._process_digit_list(escape_digits)
        res = self.execute('RECORD FILE', self._quote(filename), format, escape_digits, timeout, offset, beep)['message'][0]
        try:
            return chr(int(res))
        except:
            raise AGIError('Unable to convert message to digit: %s' % res)
    ...
    def appexec(self, application, options=''):
        ...
        application
        '''
        message = self.execute('EXEC', application, self._quote(options))
    ...
    def channel_status(self, channel=''):
        ...
        '''
        try:
           message = self.execute('CHANNEL STATUS', channel)
        except AGIHangup:
           raise
        except AGIAppError:
           message = {'message': ('-1','')}

        return int(message['message'][0])
    ...
    def get_variable(self, name):
        ...
        '''
        try:
           message = self.execute('GET VARIABLE', self._quote(name))
        except AGImessageHangup:
           message = {'message': ('1', 'hangup')}
    ...
    def get_full_variable(self, name, channel = None):
        ...
        try:
           if channel:
              message = self.execute('GET FULL VARIABLE', self._quote(name), self._quote(channel))
           else:
              message = self.execute('GET FULL VARIABLE', self._quote(name))
    ...
    def database_get(self, family, key):
        ...
        example return code: 200 message=1 (testvariable)
        '''
        family = '"%s"' % family
        key = '"%s"' % key
        message = self.execute('DATABASE GET', self._quote(family), self._quote(key))
        res, value = message['message']
        if res == '0':
            ...
            raise AGIError('Unknown exception for : family=%s, key=%s, message=%s' % (family, key, pprint.pformat(message)))
    ...
    def database_put(self, family, key, value):
        ...
        '''
        message = self.execute('DATABASE PUT', self._quote(family), self._quote(key), self._quote(value))
        res, value = message['message']
        if res == '0':
    ...
    def database_del(self, family, key):
        ...
        '''
        message = self.execute('DATABASE DEL', self._quote(family), self._quote(key))
        res, value = message['message']
        if res == '0':
    ...
    def database_deltree(self, family, key=''):
        ...
        '''
        message = self.execute('DATABASE DELTREE', self._quote(family), self._quote(key))
        res, value = message['message']
        if res == '0':
